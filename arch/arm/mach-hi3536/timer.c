...
#  undef CONFIG_ARM_LOCAL_TIMER
#  undef CONFIG_SP804_LOCAL_TIMER
#endif
...
static struct hi_timer_t hi_timer[] = {
	...
	HI_TIMER(8, INTNR_TIMER_8, REG_BASE_TIMER89),
	HI_TIMER(9, INTNR_TIMER_9, REG_BASE_TIMER89 + 0x20),
};
...
static int __cpuinit hi3536_local_timer_setup(struct clock_event_device *evt)
{
...
static void hi3536_local_timer_stop(struct clock_event_device *evt)
{
...
static struct local_timer_ops hi3536_timer_tick_ops __cpuinitdata = {
	.setup	= hi3536_local_timer_setup,
	.stop	= hi3536_local_timer_stop,
};

static void __init hi3536_local_timer_init(void)
{
	unsigned int cpu = 0;
	...
	local_timer_register(&hi3536_timer_tick_ops);
}
#endif /* CONFIG_SP804_LOCAL_TIMER */

/*****************************************************************************/
#ifdef CONFIG_ARM_LOCAL_TIMER
DEFINE_TWD_LOCAL_TIMER(twd_localtimer,
		(resource_size_t)(REG_BASE_A17_PERI
			+ REG_A17_PERI_PRI_TIMER_WDT),
		(resource_size_t)IRQ_LOCALTIMER);
#endif

/*****************************************************************************/

struct hi3536_clocksource {
	void __iomem *base;
	struct clocksource clksrc;
};

static struct hi3536_clocksource hi3536_clocksource = {0};

static inline struct hi3536_clocksource
			*to_hi3536_clksrc(struct clocksource *cs)
{
	return container_of(cs, struct hi3536_clocksource, clksrc);
}

static void hi3536_clocksource_start(void __iomem *base)
{
...
static cycle_t hi3536_clocksource_read(struct clocksource *cs)
{
	return ~readl_relaxed(to_hi3536_clksrc(cs)->base + TIMER_VALUE);
}

static notrace u32 hi3536_sched_clock_read(void)
{
	return ~readl_relaxed(hi3536_clocksource.base + TIMER_VALUE);
}

static void hi3536_clocksource_resume(struct clocksource *cs)
{
	hi3536_clocksource_start(to_hi3536_clksrc(cs)->base);
}
