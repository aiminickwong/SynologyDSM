...
u_char spi_general_get_flash_register(struct hisfc_spi *spi, u_char cmd)
{
	...
	if (DEBUG_GET_SR)
		pr_info("* Start get flash Register %#x.\n", cmd);
	...
	if (DEBUG_GET_SR)
		pr_info("* Get flash Register %#x, val[%#x]\n", cmd,
				status);
...
{
	if (DEBUG_SPI_QE)
		pr_info("SPI read if[%d] write if[%d]\n",
...
static int spi_general_qe_enable(struct hisfc_spi *spi)
{
	...
	if (DEBUG_SPI_QE)
		pr_info("* Start SPI Nor %s Quad.\n", str[op]);

	config = spi_general_get_flash_register(spi, SPI_CMD_RDCR);
	if (DEBUG_SPI_QE)
		pr_info("  Read config %#x, val[%#x]\n", SPI_CMD_RDCR,
				config);
