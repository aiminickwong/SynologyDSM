...
unsigned int stmmac_irq_num[] = {
	SYNOP_GMAC_IRQNUM,
#if (TOE_MULTI_INTR_NUM > 1)
	CONFIG_STMMAC_IRQ1NUM,
#endif
#if (TOE_MULTI_INTR_NUM > 2)
	CONFIG_STMMAC_IRQ2NUM,
#endif
#if (TOE_MULTI_INTR_NUM > 3)
	CONFIG_STMMAC_IRQ3NUM,
#endif
};
#endif
...
#define MAX_DMA_TX_SIZE	(16 * 1024)
static int dma_txsize = DMA_TX_SIZE;
module_param(dma_txsize, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(dma_txsize, "Number of descriptors in the TX list");

#define DMA_RX_SIZE 1024
#define MAX_DMA_RX_SIZE	(16 * 1024)
...
static void stmmac_verify_args(void)
{
	if (unlikely(watchdog < 0))
		watchdog = TX_TIMEO;
	if (unlikely(dma_rxsize <= 0 || dma_rxsize > MAX_DMA_RX_SIZE))
		dma_rxsize = DMA_RX_SIZE;
	if (unlikely(dma_txsize <= 0 || dma_txsize > MAX_DMA_TX_SIZE))
	...
	/* make sure dma_rxsize or dma_txsize be 2^n */
	if (dma_rxsize & (dma_rxsize - 1))
		dma_rxsize = DMA_RX_SIZE;

	if (dma_txsize & (dma_txsize - 1))
		dma_txsize = DMA_TX_SIZE;

	/* when in bypass mode, set large dma_rxsize for
	 * no FIFO overflow and better receive performance.
	 */
	if (!hitoe && (dma_rxsize == DMA_RX_SIZE))
		dma_rxsize = 4096;
}
...
static int init_dma_desc_rings(struct net_device *dev)
{
	...
	/* Add multi-packet interrupt for receive */
	for (i = 0; i < rxsize; i++) {
		struct dma_desc *p = priv->dma_rx + i;
...
#if SWITCH_MULTI_INTR
		max_ch = TNK_NUM_GMACS + TOE_MULTI_INTR_NUM;
#endif
...
#if SWITCH_MULTI_INTR
				tnkhw_rx_refill(ch - TNK_NUM_GMACS);
#else
				tnkhw_rx_refill();
#endif
			}
...
static int stmmac_open(struct net_device *dev)
{
	...
	if (unlikely((stmmac_open_ext_timer(dev, priv->tm)) < 0)) {
		pr_warn("stmmaceth: cannot attach the external timer.\n");
		priv->tm->freq = 0;
...
void stmmac_proc(struct seq_file *s)
{
	...
	for (i = 0; i < STMMAC_CPU_CORE_NUM; i++) {
...
static netdev_tx_t stmmac_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* skb_checksum_help() may linear skb in linux-3.10.y.
	 * do_tcp_sendpages() set SKBTX_SHARED_FRAG
	 *	in skb_shinfo(skb)->tx_flags.
	 * skb_checksum_help() will call __skb_linearize().
	 * so nfrags may be changed if software do checksum calculate.
	 */
...
static inline void stmmac_rx_refill(struct stmmac_priv *priv)
{
	...
	for (; (((priv->cur_rx >= priv->dirty_rx) &&
		...
		if (priv->hw->desc->get_rx_curr_flag(p + entry)) {
			...
			pr_err("%s dirty point has passed current point,line = %d\n",
					__func__, __LINE__);
		...
		if (unlikely(status == discard_frame))
			...
			if (unlikely(!skb)) {
				...
				if (priv->hw->desc->get_rx_dirty_flag(p)) {
					...
					pr_err("%s line %d dirty point should be set"
							"here is wrong.\n",
							__func__, __LINE__);
				spin_unlock_irqrestore(&priv->rxlock, flags);
				break;
...
get_next_desc:
		...
		if (priv->hw->desc->get_rx_dirty_flag(p)) {
			...
			pr_err("%s dirty point should be set,here is wrong\n",
					__func__);
...
static int stmmac_probe(struct net_device *dev)
{
	...
	if (!is_valid_ether_addr(dev->dev_addr))
		pr_warn("\tno valid MAC address for MAC %d;"
...
#if SWITCH_MULTI_INTR
	for (i = 1; i < TOE_MULTI_INTR_NUM; i++)
		free_irq(stmmac_irq_num[i], pdev);
#endif
	tnk_shutdown();
...
#if SWITCH_MULTI_INTR
	for (channel = 0; channel < 2 + TOE_MULTI_INTR_NUM; channel++) {
#else
...
/* config HI3536 all features switch register */
static void stmmac_switch_enable(void)
{
	unsigned int hi3536_features;

	hi3536_features =
		(SWITCH_DUAL_TXBUF		<< 0) |
		(SWITCH_RETRANS_TWO_PKT		<< 1) |
		(SWITCH_DUPACK_NUM		<< 2) |
		(SWITCH_RETRANS_MAX_CNT		<< 3) |
		(SWITCH_SEND_ACK		<< 4) |
		(SWITCH_REMOVE_TTX_ERR		<< 5) |
		(SWITCH_TOE_LOOPBACK		<< 6) |
		(SWITCH_RCVBUF_ADDR		<< 7) |
		(SWITCH_MULTI_INTR		<< 8) |
		(SWITCH_RTT_CACULATE		<< 9) |
		(SWITCH_IP_HEADER_ID		<< 10) |
		(SWITCH_ZERO_PROBE		<< 11) |
		(SWITCH_CORK			<< 12) |
		(SWITCH_NAGLE			<< 13) |
		(SWITCH_KEEPALIVE		<< 14) |
		(SWITCH_SEND_FIN		<< 15) |
		(SWITCH_THIN_STREAM		<< 16) |
		(SWITCH_MULTI_INTR_RETRANS	<< 17) |
		(SWITCH_RECV_LRO		<< 18) |
		(SWITCH_HW_UPDATE_WND		<< 19) |
		(SWITCH_FOUR_OFO		<< 20) |
		(SWITCH_TOE_ERR_PKT		<< 21) |
		(SWITCH_CV_FIFO			<< 22) |
		(SWITCH_PROBE_SEQ		<< 23) |
		(SWITCH_TTX_FLUSH		<< 24);

	writel(hi3536_features,
			stmmac_base_ioaddr + TNK_REG_TOE_SWITCH_CTRL_V220);
}
...
#if SWITCH_MULTI_INTR
	for (i = 1; i < TOE_MULTI_INTR_NUM; i++) {
		ret = request_irq(stmmac_irq_num[i], stmmac_interrupt,
				IRQF_SHARED, STMMAC_RESOURCE_NAME, pdev);
		if (unlikely(ret < 0)) {
			pr_err("%s: ERROR: allocating the IRQ %d (error: %d)\n",
					__func__, stmmac_irq_num[i], ret);
			goto out_free_irq;
		}
	}
#endif

	stmmac_switch_enable();

	/*  Enable all interrupts */
	writel(~0, stmmac_base_ioaddr + TNK_REG_INTR_EN);
#if SWITCH_MULTI_INTR
	writel(~0 & TNK_MASK_INTR, stmmac_base_ioaddr + TNK_REG_INTR_EN);
#if (TOE_MULTI_INTR_NUM > 1)
	writel(~0 & TNK_MASK_INTR1, stmmac_base_ioaddr + TNK_REG_INTR1_EN);
#endif
#if (TOE_MULTI_INTR_NUM > 2)
	writel(~0 & TNK_MASK_INTR2, stmmac_base_ioaddr + TNK_REG_INTR2_EN);
#endif
#if (TOE_MULTI_INTR_NUM > 3)
	writel(~0 & TNK_MASK_INTR3, stmmac_base_ioaddr + TNK_REG_INTR3_EN);
#endif
#endif
...
out_free_irq:
	for (j = 1; j < i; j++)
		free_irq(stmmac_irq_num[j], pdev);
#endif
	free_irq(SYNOP_GMAC_IRQNUM, pdev);
