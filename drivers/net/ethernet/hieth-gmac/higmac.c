...
next_phyaddr:
		...
		if (tmp == 0)
			pr_info("gmac%d phyaddr set to 0,is it right?\n",
					gmac);
		else if (tmp >= 32) {
...
static int higmac_init_hw_desc_queue(struct higmac_netdev_local *ld)
{
	...
	if (ld->tx_bq.sg_desc_offset == NULL) {
		pr_err("alloc gmac%d tx sg_desc_offset failed!\n", ld->index);
		goto error_free_pool;
	}
#endif
	higmac_hw_set_desc_queue_addr(ld);
...
#ifdef HIGMAC_RXCSUM_SUPPORTED
		if (dev->features & NETIF_F_RXCSUM) {
			int hdr_csum_done =
				rx_bq_desc->desc1.rx.header_csum_done;
			int payload_csum_done =
				rx_bq_desc->desc1.rx.payload_csum_done;
			int hdr_csum_err =
				rx_bq_desc->desc1.rx.header_csum_err;
			int payload_csum_err =
				rx_bq_desc->desc1.rx.payload_csum_err;
...
static inline void higmac_enable_rxcsum_drop(struct higmac_netdev_local *ld,
		bool drop)
{
	higmac_writel_bits(ld, drop, TSO_COE_CTRL, BIT_COE_IPHDR_DROP);
	higmac_writel_bits(ld, drop, TSO_COE_CTRL, BIT_COE_PAYLOAD_DROP);
	higmac_writel_bits(ld, drop, TSO_COE_CTRL, BIT_COE_IPV6_UDP_ZERO_DROP);
}
